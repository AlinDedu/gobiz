...
const ProductScreen = () => {
	...
	const changeAmmount = (input) => {
		if (input === 'plus') setAmmount(ammount + 1);
		if (input === 'minus') setAmmount(ammount - 1);
	};
		...
		<Wrap spacing='30px' justify='center' minH='100vh'>
			{loading ? (
				<Stack direction='row' spacing='4'>
					<Spinner mt='20' thickness='2px' speed='0.65s' emptyColor='grey.200' color='cyan.500' size='xl' />
				</Stack>
			) : error ? (
				<Alert status='error'>
					<AlertIcon />
						...
						<Stack direction={{ base: 'column', lg: 'row' }} align='flex-start'>
							<Stack pr={{ base: '0', md: 'row' }} flex='1.5' mb={{ base: '12', md: 'none' }}>
								...
								<Stack spacing='5'>
									...
									<Flex w='170px' p='5px' border='2px' borderRadius='10' borderColor='gray.200' alignItems='center'>
										<Button isDisabled={ammount <= 1} onClick={() => changeAmmount('minus')}>
											<MinusIcon />
										</Button>
										<Text mx='30px'>{ammount}</Text>
										<Button isDisabled={ammount >= product.stock} onClick={() => changeAmmount('plus')}>
											<SmallAddIcon />
										</Button>
									</Flex>
